...
After subsetting the data to create the analytical sample and recoding the variables and specifying their character (factors) one can start calculating the regression model(s).

The basic function for calculating OLS regressions is *lm*.

It is structured in the following way: `mylm <- lm(DV ~ IV + CV1 + .., data)`. 

This saves the output as a list object `mylm` which enables you to create a simple summary output using `summary(mylm)`
